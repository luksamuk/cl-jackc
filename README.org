...
* Usage
To use the compiler, one can consider the following ways:
...
- The ~cl-jackc~ package, which serves as interface to the compiler.

Please notice that this software is still beta quality and not ready
for usage.
...
After the required operations, the script ~jackc~ should be available to
...
#+begin_src bash
./run-jackc.sh files-or-dirs-or-args...
#+end_src
...
using Quicklisp), one can just load and use the default interface:
...
~:ANALYZE~ can be ignored. If so, it defaults to ~NIL~, which indicates
that a full compilation of the file(s) should be done, and not just
syntax analysis.
